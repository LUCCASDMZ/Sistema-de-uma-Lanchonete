...
describe('CaixaDaLanchonete', () => {
    ...
    const validaTeste = (formaDePagamento, resultadoEsperado, itens) => {
        const resultado = new CaixaDaLanchonete()
            ...
            const resultadoNumerico = parseFloat(resultado.replace(/[^\d,]/g, '').replace(',', '.'));
            const resultadoEsperadoNumerico = parseFloat(resultadoEsperado.replace(/[^\d,]/g, '').replace(',', '.'));
    };
    ...
    test.each([
        ['chantily', 'dinheiro', 'Item extra não pode ser pedido sem o principal', ['chantily,1', 'cafe,1']],
        ['queijo', 'credito', 'Item extra não pode ser pedido sem o principal', ['queijo,1', 'sanduiche,1']],
        ['chantily com outro item', 'credito', 'Item extra não pode ser pedido sem o principal', ['chantily,1', 'sanduiche,1']],
        ['queijo com outro item', 'debito', 'Item extra não pode ser pedido sem o principal', ['queijo,1', 'sanduiche,1']],
    ])('compra %p em %p deve resultar em %p', (_, formaDePagamento, resultadoEsperado, itens) =>
        validaTeste(formaDePagamento, resultadoEsperado, itens));
});
